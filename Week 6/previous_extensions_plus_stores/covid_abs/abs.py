...
class Simulation(object):
    def __init__(self, **kwargs):
        ...
        self.stores = []
        '''The list of store positions'''
        self.population_size = kwargs.get("population_size", 20)
        '''The number of agents'''
        self.number_of_stores = int(math.ceil((self.population_size / 1000) * kwargs.get("stores_per_thousand", 0.18)))
        '''The number of stores calculated from the stores per 1000'''
        self.store_infection_perc = kwargs.get("store_infection_rate", 0.3)
        '''The percantage to get infected in a store'''
        self.days_without_shopping = kwargs.get("days_without_shopping", 7)
        '''The grocery shopping frequency in days'''
        ...
        self.condition_perc = kwargs.get('condition_perc', 0.4)
        '''Amount of people with medical preconditions'''
    ...
    def create_agent(self, status, infectiousness):
        """
        ...
        days_without_shopping = np.random.randint(0, self.days_without_shopping)
        self.population.append(Agent(x=x, y=y, age=age, status=status, infected_status=infectiousness,
                                     social_stratum=social_stratum, days_without_shopping=days_without_shopping))
    ...
    def initialize(self):
        ...
        for i in np.arange(0, int(self.population_size * self.initial_infected_perc)):
            self.create_agent(Status.Infected, InfectionSeverity.Infectious)

        # Initial immune population
        for i in np.arange(0, int(self.population_size * self.initial_immune_perc)):
            self.create_agent(Status.Recovered_Immune, InfectionSeverity.Asymptomatic)

        # Initial susceptible population
        for i in np.arange(0, self.population_size - len(self.population)):
            self.create_agent(Status.Susceptible, InfectionSeverity.Asymptomatic)
        ...
        # add a number of stores
        self.stores = []
        for i in range(self.number_of_stores):
            x, y = self.store_position()
            self.stores.append([x, y])
        self.stores = np.array(self.stores)
    ...
    def contact(self, agent1, agent2):
        ...
        # incubation period
        if agent1.status == Status.Susceptible and agent2.status == Status.Infected and \
                agent2.infected_status == InfectionSeverity.Infectious:
            contagion_test = np.random.random()
            #agent1.infection_status = InfectionSeverity.Exposed
            if agent1.is_shopping and agent2.is_shopping:
                if contagion_test <= self.store_infection_perc:
                    agent1.status = Status.Infected
                    agent1.infected_status = InfectionSeverity.Exposed
                    # counter to calculate R, the effective reproduction number per day
    ...
    def move(self, agent, triggers=[]):
        ...
        for trigger in triggers:
            if trigger['condition'](agent):
                ...
                return
        # a check build for moving to the shop and back
        if agent.days_without_shopping > self.days_without_shopping:
            agent.days_without_shopping = 0
            agent.is_shopping = True
            agent.pre_shopping_position = (agent.x, agent.y)
            best_shop = (self.length*10, self.height*10)
            for store in self.stores:
                if abs(math.sqrt((best_shop[0] - agent.x)**2 + (best_shop[1] - agent.y)**2)) > \
                        abs(math.sqrt((agent.x - store[0]) ** 2 + (agent.y - store[1]) ** 2)):
                    best_shop = (store[0]+ np.random.uniform(-1.0,1.0,1), store[1]+ np.random.uniform(-1.0,1.0,1))
            agent.x, agent.y = best_shop
        else:
            if agent.is_shopping:
                agent.is_shopping = False
                agent.x, agent.y = agent.pre_shopping_position
            else:
                agent.days_without_shopping += 1
    ...
    def update(self, agent):
        ...
        if agent.status == Status.Infected:
            ...
            medical_condition_sub = np.random.random()

            ## incubation period
            if agent.infected_status == InfectionSeverity.Exposed:
                agent.exposed_time += 1
            if agent.exposed_time >= 3:
                agent.exposed_time = 0
                agent.infected_status = InfectionSeverity.Infectious

            if agent.infected_status == InfectionSeverity.Infectious:
                # medical conditions
                if self.condition_perc > medical_condition_sub:
                    agent.infected_status = InfectionSeverity.Hospitalization
                elif age_hospitalization_probs[indice] > teste_sub:
                    agent.infected_status = InfectionSeverity.Hospitalization
                ...
                elif age_severe_probs[indice] > teste_sub:
                    agent.infected_status = InfectionSeverity.Severe
                    self.get_statistics()
                    if self.statistics['Severe'] + self.statistics['Hospitalization'] >= self.critical_limit:
                        agent.status = Status.Death
    ...
    def get_statistics(self, kind='info'):
        ...
        if self.statistics is None:
            ...
            ## Added incubation period and exposed state
            for infected_status in filter(lambda x: x != InfectionSeverity.Asymptomatic, InfectionSeverity):
            ...
            # saved values of the effective reproduction number R
            self.statistics["Stat_R"] = np.sum(
                    [a.number_infected for a in self.population if a.status == Status.Infected]) / self.population_size

        return self.filter_stats(kind)

    def filter_stats(self, kind):
        if kind == 'info':
            return {k: v for k, v in self.statistics.items() if not k.startswith('Q') and k not in ('Business','Government','Stat_R')}
        elif kind == 'ecom':
            return {k: v for k, v in self.statistics.items() if k.startswith('Q') or k in ('Business','Government')}
        elif kind == 'extra':
            return {k: v for k, v in self.statistics.items() if k.startswith('Stat')}
        else:
...
class MultiPopulationSimulation(Simulation):
    ...
    def get_statistics(self, kind='info'):
        if self.statistics is None:
            ...
            # saved values for the effective reproduction number R
            for simulation in self.simulations:
                self.statistics["Stat_R"] = np.sum(
                    [a.number_infected for a in simulation.get_population() if a.status == Status.Infected])
            self.statistics["Stat_R"] /= self.total_population
